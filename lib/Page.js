;(function (root, factory) {
	...
	Page.prototype.navigate = function navigate(url, options) {
		...
		if (options.trigger){
			this.router.navigate(url, {trigger: false, replace: true});
	...
	Page.prototype.use = function use(middleware) {
		if (!middleware) throw new Error('Middleware cannot be null or undefined');
		...
		var route = function route() {
			...
			_.each(args, function(param){
				if ( _.isString(param) && param.indexOf('=') > -1 ){
					...
					if ( aParams.length > 0){
						_(aParams).each((aParam)=>{
							var p = aParam.split("=");
							if ( p.length > 0 ){
								var key = p.shift();
								queryParams[key] = p.join("=");
							}
						});
					}
				}
			...
			_(middlewares).forEach(function (aMiddleware) {
				...
				var arrayOfMiddlewares = _.isArray(aMiddleware) ? aMiddleware : [aMiddleware];
				_.forEach(arrayOfMiddlewares, function (aMiddleware) {
					if (typeof aMiddleware === 'function') {
						actions.push(function (cb) {
							aMiddleware(context, function(err){
								return cb(err);
